...
#'                   pair.
#' @param gname name of the grouping to distinguish from other membership data.
...
#'
#' @return return \code{data.frame} with row for earch group-term pair
#' @export
#'
#' @examples
#' library(igraph)
...
run_enrichment<-function(membership,annotation,gname,printTwoSided=TRUE,
    ...
    if(any(is.na(match(c('names','termID','termName'),names(annotation))))){
        stop("Annotation should contains columns 'names', ",
             "'termID' and 'termName'.\n")
    }
