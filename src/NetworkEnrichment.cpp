...
void NetworkEnrichment::calculateFDR( int Study, int indexA, int indexB, int indexC ){
  ...
  if( Study == 2 ){//calculateOverlapinCommunities
    ...
    for(k=0; k<K; k++){
      ...
      pv_sortedDT.push_back  ( pairDoubInt(p_valuesDT[(i*M)+m], k) );
      pv_sortedT.push_back   ( pairDoubInt(p_valuesT [(i*M)+m], k) );
...
void NetworkEnrichment::overlapinComsHypergeometricTest(int indexA, int indexB){
  ...
  for(k=0, a=0; a<As; a++){
	...
	int maxMU        = 0;
	  ...
	  //calculate p-values, depletion one-sided
	  p_valuesD[(k*M)+m]   = (double)p_valueD;

	  //calculate p-values, enrichment two-sided
	  p_valuesT[(k*M)+m]   = (double)p_valueT;

	  //calculate p-values, depletion two-sided
	  p_valuesDT[(k*M)+m]  = (double)p_valueDT;
...
int NetworkEnrichment::calculateOverlapinCommunities( int indexA, int indexB, const char* outdir, const char* ext, bool runFDR ){
  ...
  if( (Alist.size() >= 2) && (indexA >= 0) && (indexA < Alist.size())
    ...
    p_valuesDT   = (double*)calloc(A*B*M,sizeof(double));
    padjustedDT  = (double*)calloc(A*B*M,sizeof(double));
...
void NetworkEnrichment::printOverlapinCommunities( int indexA, int indexB, const char *outdir, const char *ext, bool printFDR){
  ...
  for(k=0, a=0; a<A; a++){
	...
	for(i=0; i<SIGMASIZE; i++){
	  ...
	  if( p_valuesDT [(k*M)+m] <= bonferroni[i] ){ starsDT   += "*";  }
	...
	*/
	...
	if( printTwoSided ){
	  sprintf(buffer2,"%G, %G, %s",p_valuesT [(k*M)+m],padjustedT [(k*M)+m],starsET.c_str());
	  sprintf(buffer3,"%G, %G, %s",p_valuesDT[(k*M)+m],padjustedDT[(k*M)+m],starsDT.c_str());
	} else {
...
int NetworkEnrichment::getN(){ return N; }

int NetworkEnrichment::getM(){ return M; }

int NetworkEnrichment::getF(){ return F; }

int NetworkEnrichment::getFsize(int index){ return Fsize[index]; }

int NetworkEnrichment::getSigmaSize(){ return SIGMASIZE; }

double NetworkEnrichment::calBonferroni(int i, double dem){

  return sigma[i]/(double)(dem);//Bonferroni at sigma[i]

}

double NetworkEnrichment::getPvalue(int i){ return p_values[i]; }

double NetworkEnrichment::getPvalueT(int i){ return p_valuesT[i]; }

double NetworkEnrichment::getPvalueD(int i){ return p_valuesD[i]; }

double NetworkEnrichment::getPvalueDT(int i){ return p_valuesDT[i]; }

double NetworkEnrichment::getPadjusted(int i){ return padjusted[i]; }

double NetworkEnrichment::getPadjustedT(int i){ return padjustedT[i]; }

double NetworkEnrichment::getPadjustedD(int i){ return padjustedD[i]; }

double NetworkEnrichment::getPadjustedDT(int i){ return padjustedDT[i]; }

double NetworkEnrichment::getOverlap(int i){ return overlap[i]; }

double NetworkEnrichment::getPchi2(int i){ return p_chi2[i]; }

double NetworkEnrichment::getPadjustedChi2(int i){ return padjustedCHI2[i]; }

int NetworkEnrichment::getAnnoK(int index, int i){ return ANNOS[index][i].K; }

string NetworkEnrichment::getAnnoID(int index, int i){
  string ostring = ANNOS[index][i].annoID;
  return ostring;
}

string NetworkEnrichment::getAnnoName(int index, int i){
  string ostring = ANNOS[index][i].annoID;
  return ostring;
}

string NetworkEnrichment::getAnnoDes(int index, int i){
  string ostring = ANNOS[index][i].annoDES;
  return ostring;
}

int NetworkEnrichment::getCom0(int i){ return std::get<0>(COMS[i]); }

int NetworkEnrichment::getCom1(int i){ return std::get<1>(COMS[i]); }

int NetworkEnrichment::getCom2(int i){ return std::get<2>(COMS[i]); }

double NetworkEnrichment::getMuCab(int i){ return muCab[i]; }

int NetworkEnrichment::getKOFFSET(){ return KOFFSET; }

int NetworkEnrichment::getANNOindex(){ return ANNOindex; }

int NetworkEnrichment::getPermute(int i){ return permute[i]; }
